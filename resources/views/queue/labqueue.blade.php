                        ...
                        <div class="flex justify-start p-2 bg-gray-600 flex-wrap">
                            @foreach ($queuesdoc as $key => $queue)
                                <div class="block p-6 rounded-lg shadow-lg bg-white max-w-sm min-w-fit m-2">
                                    <h2 class="text-gray-900 text-4xl leading-tight font-bold"> -> {{ $queue->lab_assistant->room->room_no }}</h2>
                                    ...
                                    <p class="text-gray-700 text-xs mb-1">
                                        {{ $queue->lab_assistant->name }} | {{ $queue->student->id }}
                                    </p>
                                </div>
                            @endforeach
                                ...
                                <div class="overflow-hidden">
                                    <table class="min-w-full">
                                        <thead class="border-b">
                                            <tr>
                                                <th scope="col" class="text-sm font-larg text-gray-900 px-6 py-4 text-left">
                                                    #
                                                </th>
                                                <th scope="col" class="text-sm font-larg text-gray-900 px-6 py-4 text-left">
                                                    STUDENT ID
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($queues as $key => $que)
                                                <tr class="border-b">
                                                    ...
                                                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                                        {{ $que->student->first_name }} . | . {{ $que->student->id }}
                                                    </td>
                                                    </td>
                                                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                                        {{-- {{$rooms->room_no}} --}}
                                                        {{-- {{ $que->lab_assistant->room->room_no }} --}}
